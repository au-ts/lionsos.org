...
You can also ping the firewall interface IP addresses directly from each
namespace.

Before running this test, make sure ping response is enabled in the firewall
...
#### ICMP reject rules (UDP and ICMP)

If a firewall rule action is set to `reject` for ICMP or UDP traffic, the
firewall sends an ICMP `destination unreachable` response back to the sender.

To test this, first configure a rule in the webserver that matches the target
traffic and sets the action to `reject`, then run:

```sh
# ICMP reject test: ext --> int
ip netns exec ext ping ${INT_HOST_IP}

# ICMP reject test: int --> ext
ip netns exec int ping ${EXT_HOST_IP}

# UDP reject test: ext --> int
ip netns exec ext nc -u ${INT_HOST_IP} ${TEST_PORT}

# UDP reject test: int --> ext
ip netns exec int nc -u ${EXT_HOST_IP} ${TEST_PORT}
```

For ICMP, `ping` should show `Destination Host Unreachable` responses from the
firewall. For UDP, the sender should receive an ICMP `destination unreachable`
generated by the firewall (visible in packet captures and firewall logs).
...
You can also test ICMP timeout behaviour by setting a very low TTL in the ping
request so the packet expires at the firewall hop.
